...
このドキュメントは、スプレッドシートCMS型QAサイト（Astro + Cloudflare Pages + D1）を
ローカルで開発・確認し、Cloudflareにデプロイするための人間向け手順書です。
アーキテクチャの背景や設計判断については `SKILL.md` を参照してください。
...
3. 左側の歯車アイコン（プロジェクトの設定）→「スクリプト プロパティ」で
   `REFRESH_TOKEN` を追加し、手順9で本番用に発行したものと**同じ値**を設定
