...
8. **Formatting, type checking, and lint** — set up Prettier (with `prettier-plugin-astro`), `astro check`, `wrangler types`, and ESLint (`typescript-eslint` + `eslint-plugin-astro`) per `references/quality.md` before considering the build done. Run the combined `pnpm verify` script (`format:check` → typecheck → lint) and treat the build as incomplete until it's clean.
...
- [ ] `pnpm verify` (format check + typecheck + lint, see `references/quality.md`) passes clean — not "pre-existing errors only"
