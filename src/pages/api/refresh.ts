...
export const POST: APIRoute = async ({ request }) => {
    ...
    if (authorization !== `Bearer ${env.REFRESH_TOKEN}`) {
        return json(
            { message: "認証に失敗しました。更新トークンをご確認ください。" },
            401,
        );
    }
    ...
    try {
        ...
        const query = ranges
            .map(
                (range) =>
                    `ranges=${encodeURIComponent(quoteSheetName(range))}`,
            )
            .join("&");
        ...
        if (valueRanges.length !== ranges.length) {
            throw new Error(
                "Google Sheetsから必要なシートをすべて取得できませんでした。",
            );
        }

        const companies = parseCompanies(valueRanges[0]?.values ?? []);
        const slugs = new Set(companies.map((company) => company.slug));
        const orphanedTabs = individualTabs.filter(
            (tab) => !slugs.has(tab.slice("個別QA_".length)),
        );
        if (orphanedTabs.length > 0) {
            return json(
                {
                    message: `設定に識別子がない個別QAタブがあります: ${orphanedTabs.join("、")}`,
                },
                400,
            );
        }
        ...
        return json({
            message: `${companies.length}社・${commonQa.length + companyQa.length}件のQAを反映しました`,
        });
    } catch (error) {
...
export const ALL: APIRoute = () =>
    json(
        { message: "このエンドポイントはPOSTメソッドのみ利用できます。" },
        405,
        { Allow: "POST" },
    );
...
function parseCompanies(values: string[][]): CompanyRow[] {
    ...
    return rows.map((row, index) => {
        ...
        if (!slug || !IDENTIFIER_PATTERN.test(slug)) {
            throw new Error(
                `設定の${index + 2}行目: 識別子は半角英数字で入力してください。`,
            );
        }
        if (seen.has(slug))
            throw new Error(`設定に重複した識別子があります: ${slug}`);
...
function parseQa(values: string[][], sheetName: string): QaRow[] {
    return rowsAsObjects(values, sheetName).map((row, index) => {
        ...
        if (!category || !question || !answer) {
            throw new Error(
                `${sheetName}の${index + 2}行目: カテゴリ・質問・回答は必須です。`,
            );
        }
        return {
            category,
...
function rowsAsObjects(
    ...
    if (values.length === 0)
        throw new Error(`${sheetName}シートにヘッダー行がありません。`);
...
async function getAccessToken(rawKey: string): Promise<string> {
    ...
    try {
        ...
        throw new Error(
            "GOOGLE_SERVICE_ACCOUNT_KEYが正しいJSONではありません。",
        );
    }
    if (!serviceAccount.client_email || !serviceAccount.private_key) {
        throw new Error(
            "GOOGLE_SERVICE_ACCOUNT_KEYに必要な認証情報がありません。",
        );
    }
    ...
    if (!response.ok || !payload.access_token) {
        throw new Error(
            `Google認証に失敗しました: ${payload.error_description ?? response.statusText}`,
        );
    }
...
async function googleFetch<T>(url: string, accessToken: string): Promise<T> {
    ...
    if (!response.ok) {
        ...
        throw new Error(
            `Google Sheetsの取得に失敗しました: ${payload?.error?.message ?? response.statusText}`,
        );
    }
